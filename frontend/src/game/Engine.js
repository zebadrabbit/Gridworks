...
export class Item {
  ...
  update() {
    if (this.currentConveyor) {
      ...
      this.sprite.x =
        this.currentConveyor.x * this.engine.gridSize +
        this.currentConveyor.length * this.engine.gridSize * this.progress;
...
export class Conveyor {
  ...
  update() {
    ...
    this.line.clear();
    this.line.lineStyle(3, 0x00ff00);
    const offset = (time % 10) * (this.engine.gridSize / 10);
    this.line.moveTo(offset, 0);
    this.line.lineTo(offset + this.engine.gridSize, this.engine.gridSize);
...
export class GameEngine {
  ...
  setupInteractions() {
    ...
    this.app.stage.on('pointerdown', (event) => {
      const worldX =
        (event.data.global.x - this.app.screen.width / 2 - this.camera.x) / this.camera.zoom;
      const worldY =
        (event.data.global.y - this.app.screen.height / 2 - this.camera.y) / this.camera.zoom;
      ...
      const clickedBuilding = this.buildings.find(
        (b) =>
          worldX_grid >= b.x &&
          worldX_grid < b.x + b.data.dimensions.width &&
          worldY_grid >= b.y &&
          worldY_grid < b.y + b.data.dimensions.length,
      );
  ...
  addBuilding(type, x, y) {
    ...
    const isOccupied = this.buildings.some((b) =>
      footprint.some(
        (p) =>
          p.x >= b.x &&
          p.x < b.x + b.data.dimensions.width &&
          p.y >= b.y &&
          p.y < b.y + b.data.dimensions.length,
      ),
    );
