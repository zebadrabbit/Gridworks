...
// One pass sets every in-radius chunk, so a node at a given position has nothing left to do
// afterwards — `_fog` caches the position it was last revealed from and skips it thereafter,
// which keeps the steady state at one integer compare per node. Without that cache this costs
// nodes x bbox every tick forever: measured at +12s of stall on an 8h offline catch-up with
// only 200 buildings, and minutes at a few thousand. `_fog` is transient like `_net`.
...
`_fog` must be stripped on load, exactly like `_net`: a save carrying a stale `_fog` whose
`explored` array got defaulted or repaired would skip every node and leave the map black. Add
`delete n._fog;` beside the existing `delete n._net;` line in `normalizeSave`.
